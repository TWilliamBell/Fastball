...
def showtext(string,window):
    screentext=visual.TextStim(win=window, color='black', height=0.5, \
text=string, alignHoriz='center')# CHANGE THE HEIGHT VALUE TO ALTER THE SIZE OF YOUR TEXT
...
prepwin = visual.Window(fullscr=False, monitor="testMonitor",\
...
fastballwin=visual.Window(fullscr=False,monitor="testMonitor",units="deg",\
color=[1,1,1])
...
for cycle in stimlist:
    for s in cycle:
        s.size=(5,5)# ############## CHANGE THE VALUES HERE TO ALTER THE SIZE OF YOUR IMAGES #################
        s.win=fastballwin
